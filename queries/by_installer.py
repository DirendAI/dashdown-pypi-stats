...
def by_installer(params, connect):
    """Which tool pulled the package: pip, uv, poetry, a mirror, etc.
    ...
    df = breakdown(connect, proj, start, end,
        cache_sql="SELECT installer, downloads FROM pypi_break_installer "
                  "WHERE project = ${proj} AND win = ${win} "
                  "ORDER BY downloads DESC LIMIT 8",
        live_sql="SELECT if(installer = '', 'unknown', installer) AS installer, sum(count) AS downloads "
                 "FROM pypi.pypi_downloads_per_day_by_version_by_installer_by_type "
                 "WHERE project = ${project} AND date BETWEEN ${start} AND ${end} "
                 "GROUP BY installer ORDER BY downloads DESC LIMIT 8")
    return top_slices(df, "installer")
